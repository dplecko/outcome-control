...
Consider the structural causal model (SCM) introduced in Ex. 1 of the paper: 
\begin{align}
...
We look at the policy $D^*$ obtained from Algorithm 1:
\begin{align}
...
Finally, to perform the utilitarian approach, we have from the disparity in $D^{CF}$ that: \begin{align}
  M = 0.
\end{align} 
Therefore, following Algorithm 3 with method = UT, we compute:
```{r}
...
\end{align}
The true population value also equals $0\%$.
